...
png("Plot4.png",width = 480, height = 480) ##Creating a graphics device .png with 480x480.
par(mfcol=c(2,2))
...
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("black","red","blue"),lwd=1,lty=1)##set legend
plot(subpowerdata$timestamp,subpowerdata$Voltage, type="l",xlab= "datetime", ylab= "Voltage")##plot time series Voltage
plot(subpowerdata$timestamp,subpowerdata$Global_reactive_power, type="l",xlab= "datetime", ylab= "Global_reactive_power")##plot time series Global Reactive Power
dev.off()
